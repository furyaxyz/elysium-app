...
func CmdPayForBlob() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Pay for a data blob to be published to the Elysium blockchain",
		Long: "Pay for a data blob to be published to the Elysium blockchain. " +
...
func broadcastPFB(cmd *cobra.Command, blob *types.Blob) error {
	...
	// See https://github.com/furyaxyz/elysium-app/issues/1041
