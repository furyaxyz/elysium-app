...
Currently, commitments in Elysium are dependent on the size of the square. The following diagram visualizes how this can happen. The visualizations in this ADR are without parity shares/erasure encoding.
...
In [`CreateCommitment`](https://github.com/elysiumorg/elysium-app/blob/8b9c4c9d13fe0ccb6ea936cc26dee3f52b6f6129/x/blob/types/payforblob.go#L111-L117), we calculate the commitment of the subtree roots with [`HashFromByteSlices`](https://github.com/elysiumorg/elysium-core/blob/v0.34.x-elysium/crypto/merkle/tree.go#L7-L21). This resulting Merkleroot is `Com`.
A square of 8x8 results in the following subtree roots: `H1 C D`
[`HashFromByteSlices`](https://github.com/elysiumorg/elysium-core/blob/v0.34.x-elysium/crypto/merkle/tree.go#L7-L21) results now in the Merkleroot `Com'`, which is not equal to `Com`.
...
- [rollmint/adr-007](https://github.com/elysiumorg/rollmint/blob/cb5c7440a8e879778e71097e254c3dd692c39d14/docs/lazy-adr/adr-007-header-commit-to-shares.md)
...
To implement this decision, you need to change [`CreateCommitment`](https://github.com/elysiumorg/elysium-app/blob/8b9c4c9d13fe0ccb6ea936cc26dee3f52b6f6129/x/blob/types/payforblob.go#L111-L117).
In detail, [`powerOf2MountainRange`](https://github.com/elysiumorg/elysium-app/blob/0c81704939cd743937aac2859f3cb5ae6368f174/x/payment/types/payfordata.go#L142) should take `msgMinSquareSize` as an argument instead of `squareSize`.
...
1. A Rollup can include the commitment in the block header *before* posting to Elysium because it is size-independent and does not have to wait for Elysium to confirm the square size. In general, the roll-up needs access to this commitment in some form to verify a message inclusion proof guaranteeing data availability, which Rollmint currently does not have access to.
2. In turn, this would serve as an alternative to [rollmint/adr-007](https://github.com/elysiumorg/optimint/blob/main/docs/lazy-adr/adr-007-header-commit-to-shares.md)
3. Here is one scheme on how a Rollup might use this new commitment in the block header. Let's assume a Rollup that looks like this:
  BH1 <-- BH2 <-- BH3 <-- BH4
  The Messages that are submitted to Elysium could look like this:
  Message 1: B1
...
5. So far, a full node in Rollmint downloads new blocks from the DA Layer after each Elysium block, coupled tightly for syncing. With this approach, we can send blocks over the p2p Layer giving a soft commit to full nodes. Then, they would receive the hard commit after verifying a message inclusion proof without the need to download the blocks anymore. **P2P Blocksync** You could also achieve this by saving multiple commits for each possible square size in the block header.
    1. P2P Blocksync allows a Rollmint full node to run a Elysium light node and not a Elysium full node.
    2. It allows the Rollup node to continue running after Elysium halts, relying on soft commits with no data availability.
    3. It gives the Rollup the option to run asynchronously to Elysium because you don't have to wait for new Elysium blocks/commitments of the messages.
6. Combining P2P Blocksync and the scheme in 3, we could have multiple roll-up blocks in one Elysium block. It could look like this:
  ![multiple-blocks](./assets/multiple-blocks.png)
7. When submitting a message to Elysium, you only sign the message over one commitment and not all square sizes.

We should note that Rollups can decide to do this scheme without changing the core-app apart from Number 4.

## Positive elysium-app changes
...
- Simplifies the creation of PFBs because users don't need to create commitments for multiple square sizes.
  ...
  - Simplifies the malleation process because this ADR enables a future refactor to remove WireMsgPayForBlob entirely ([issue](https://github.com/elysiumorg/elysium-app/issues/951)). Previously multiple signatures were included in a WireMsgPayForBlob and only one was used to construct the MsgPayForBlob that ended up on-chain but this ADR results in only one signature needed on the wrapping SDK message and the SDK message that ends up on-chain.
  - This renders the following issues obsolete:
    - <https://github.com/elysiumorg/elysium-app/issues/236>
    - <https://github.com/elysiumorg/elysium-app/issues/727>
- Simplifies arranging the square.
  - Currently, prepare proposal performs [`estimateSquareSize`](https://github.com/rootulp/elysium-app/blob/6f3b3ae437b2a70d72ff6be2741abb8b5378caa0/app/estimate_square_size.go#L98-L101) prior to splitting PFBs into shares because the square size is needed to malleate PFBs and extract the appropriate message share commitment for a particular square size. Since malleation no longer requires a square size, it may be possible to remove square size estimation which renders the following issues obsolete:
    - <https://github.com/informalsystems/audit-elysium/issues/12>
    - <https://github.com/informalsystems/audit-elysium/issues/24>
