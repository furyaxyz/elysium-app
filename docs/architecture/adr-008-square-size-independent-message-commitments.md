...
In [`CreateCommitment`](https://github.com/furyaxyz/elysium-app/blob/8b9c4c9d13fe0ccb6ea936cc26dee3f52b6f6129/x/blob/types/payforblob.go#L111-L117), we calculate the commitment of the subtree roots with [`HashFromByteSlices`](https://github.com/furyaxyz/elysium-core/blob/v0.34.x-elysium/crypto/merkle/tree.go#L7-L21). This resulting Merkleroot is `Com`.
A square of 8x8 results in the following subtree roots: `H1 C D`
[`HashFromByteSlices`](https://github.com/furyaxyz/elysium-core/blob/v0.34.x-elysium/crypto/merkle/tree.go#L7-L21) results now in the Merkleroot `Com'`, which is not equal to `Com`.
...
- [rollmint/adr-007](https://github.com/furyaxyz/rollmint/blob/cb5c7440a8e879778e71097e254c3dd692c39d14/docs/lazy-adr/adr-007-header-commit-to-shares.md)
...
To implement this decision, you need to change [`CreateCommitment`](https://github.com/furyaxyz/elysium-app/blob/8b9c4c9d13fe0ccb6ea936cc26dee3f52b6f6129/x/blob/types/payforblob.go#L111-L117).
In detail, [`powerOf2MountainRange`](https://github.com/furyaxyz/elysium-app/blob/0c81704939cd743937aac2859f3cb5ae6368f174/x/payment/types/payfordata.go#L142) should take `msgMinSquareSize` as an argument instead of `squareSize`.
...
2. In turn, this would serve as an alternative to [rollmint/adr-007](https://github.com/furyaxyz/optimint/blob/main/docs/lazy-adr/adr-007-header-commit-to-shares.md)
...
- Simplifies the creation of PFBs because users don't need to create commitments for multiple square sizes.
  ...
  - Simplifies the malleation process because this ADR enables a future refactor to remove WireMsgPayForBlob entirely ([issue](https://github.com/furyaxyz/elysium-app/issues/951)). Previously multiple signatures were included in a WireMsgPayForBlob and only one was used to construct the MsgPayForBlob that ended up on-chain but this ADR results in only one signature needed on the wrapping SDK message and the SDK message that ends up on-chain.
  - This renders the following issues obsolete:
    - <https://github.com/furyaxyz/elysium-app/issues/236>
    - <https://github.com/furyaxyz/elysium-app/issues/727>
