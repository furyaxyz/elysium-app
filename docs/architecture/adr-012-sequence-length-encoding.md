...
The sequence length is written as a varint in both sparse shares and compact shares. In compact shares, the varint is padded to occupy 4 bytes because 4 bytes can contain the maximum possible sequence length (assuming a 512 byte share and a max square size of 128). The fixed 4 byte length enables the compact share writer to write the contents of sparse shares (i.e. transaction data) before it writes the sequence length. See [here](https://github.com/elysiumorg/elysium-app/blob/76153bf7f3263734f31e7afd84f1e48a2f573599/pkg/shares/split_compact_shares.go#L132-L145) and [here](https://github.com/elysiumorg/elysium-app/blob/76153bf7f3263734f31e7afd84f1e48a2f573599/pkg/shares/split_compact_shares.go#L113).
...
## Option A: Remove the 4 bytes of padding from compact shares [elysium-app##1106](https://github.com/elysiumorg/elysium-app/issues/1106)
...
## Option B: Pad the sequence length to 4 bytes for sparse shares [elysium-app#1092](https://github.com/elysiumorg/elysium-app/issues/1092)
